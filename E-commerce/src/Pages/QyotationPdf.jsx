...
//   const [quotation, setQuotation] = useState(null);
  ...
  const numberToWords = (num) => {
    ...
    let str = "";
    str += +n[0] ? (a[+n[0]] || b[n[0][0]] + " " + a[n[0][1]]) + " Crore " : "";
    str += +n[1] ? (a[+n[1]] || b[n[1][0]] + " " + a[n[1][1]]) + " Lakh " : "";
    str += +n[2] ? (a[+n[2]] || b[n[2][0]] + " " + a[n[2][1]]) + " Thousand " : "";
    str += +n[3] ? (a[+n[3]] || b[n[3][0]] + " " + a[n[3][1]]) + " Hundred " : "";
    str += +n[4] ? (str !== "" ? "and " : "") + (a[+n[4]] || b[n[4][0]] + " " + a[n[4][1]]) + " " : "";
    return str.trim() + " Rupees Only";
  };

  const getProductImg = (img) =>
    img?.startsWith("http") ? img : `http://localhost:8007${img}`;

  return (
    <div id={`quotation-pdf-${quotation._id}`} className="p-5 shadow bg-white rounded-4 mx-auto" style={{ maxWidth: "750px", background: "#fff", color: "#000" }}>
             {/* Top Section */}
